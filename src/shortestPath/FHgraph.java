...
public class FHgraph<E>
{
    ...
    // algorithms
    public boolean dijkstra(E x, ArrayList<E> avoided)
    {
        FHvertex<E> w, s, v;
        ...
        while( !partiallyProcessedVerts.isEmpty() )
        {
            ...
            if(avoided.contains(v.data))      //Checking if v is to be avoided or not
            {
                continue;
            }
            ...
            for (edgeIter = v.adjList.iterator(); edgeIter.hasNext(); )
            {
                edge = edgeIter.next();
                w = edge.first;
                if(avoided.contains(w.data))         //Checking if w is to be avoided or not
                {
                    continue;
                }
                costVW = edge.second;
    ...
    public boolean showShortestPath(E x1, E x2, ArrayList<E> avoided)
    {
    ...
    public boolean showDistancesTo(E x, ArrayList<E> avoided)
    {

        Iterator< FHvertex<E> > iter;
        FHvertex<E> vert;
        //added condition if city specified is to be avoided
        if (!dijkstra(x, avoided))
            return false;
